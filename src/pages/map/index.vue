          ...
          <div class="h-full flex justify-between px-[4vw]">
            ...
            <div v-if="isAllyTeamDefeated" class="fixed inset-0 flex items-start justify-center z-50 pt-[5vh] h-[20vh] ">
              <div class="flex flex-col justify-between items-center bg-white p-8 rounded shadow-md w-[30vw]  ">
                <h2 class="text-[1.5vw] font-semibold mb-4">RIP</h2>
            ...
            <div class="flex flex-col justify-center  items-start gap-y-[1vw] relative">
              <FightSelectedCard v-for="ally in fightStore.fightData.allies" :key="ally.id" class="border-2 border-green-600 shadow-xl rounded-xl bg-gray-300/20 h-[30vh] w-[14vw] cursor-pointer hover:scale-105 transition duration-700" :class="{'ml-[2vw] transition duration-700': selectedAttackingCard === ally, 'ml-0 transition duration-700': selectedAttackingCard != ally }" :imageSelected="ally.cards.image" @click="selectAttackingCard(ally)">
            ...
            <div class="w-full flex items-end gap-x-[1.4vw] text-[4vw] text-white pb-[20vh]" :class="{'justify-end': fightStore.fightData.player === 'Bad Guys', 'justify-start ': fightStore.fightData.player === 'Cats'}">
              <span :class="{'text-red-600  pr-[3vw]': fightStore.fightData.player === 'Bad Guys', 'text-green-600 pl-[4vw]': fightStore.fightData.player === 'Cats'}"><div v-if="fightStore.fightData.player === 'Cats'">Your turn</div><div v-if="fightStore.fightData.player === 'Bad Guys'">Ennemy turn</div></span>
            </div>
            <div class="flex flex-col justify-center items-end gap-y-[1vw] relative">
              <FightSelectedCard v-for="ennemy in fightStore.fightData.ennemies" :key="ennemy.id" class="border-2 border-red-600 rounded-xl h-[30vh] w-[12vw] " :class="{'transition hover:scale-105 hover:bg-red-600 hover:border-yellow-600 cursor-pointer': fightStore.fightData.player === 'Cats', 'ml-[2vw] transition duration-700': selectedAttackingCard === ennemy, 'ml-0 transition duration-700': selectedAttackingCard != ennemy}" :imageSelected="'/_nuxt' + ennemy.image" @click="selectDefendingCard(ennemy)">
                <div class="absolute bottom-0 z-10 w-full">
                  <div class="relative">
                    <div>
                      <div
